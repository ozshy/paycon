# paycon_mult_210204.R replacing id
# paycon_mult_200524.R adding sig levels to Table 4 (Larry's final revision)
...
library(AER)# to find p-values of multinom (NNET)
...
resp_adoption_3 = readRDS("resp_adoption_3_210204.rds")# list of respondents (list of id) who adopted cash, credit, debit for 3 years
...
length(unique(pay3$id))# num resp
...
# sorting by priorities: id, then date, and then time
pay46 = pay45[order(pay45$id, pay45$date, pay45$time), ]
nrow(pay46)
head(pay46[,c("id", "date", "time"), ])
tail(pay46[,c("id", "date", "time"), ])
names(pay46)

# Adding a new variable indicating a respondent's transaction number according to data an time
pay47 = pay46 %>% group_by(id) %>% mutate(num = 1:n())
names(pay47)
head(pay47[,c("id", "date", "time", "num"), ], 20)
tail(pay47[,c("id", "date", "time", "num"), ], 20)
...
serial_max_by_resp = pay49 %>% group_by(id) %>% summarise("serial_max" = max(serial))
nrow(serial_max_by_resp) # should be equal num resp
head(serial_max_by_resp)
#
pay50 = merge(x=pay49, y=serial_max_by_resp, all.x = T)
dim(pay50)
length(unique(pay50$id))
head(pay50[ c("id", "serial", "serial_max")])
tail(pay50[ c("id", "serial", "serial_max")])

# creating new var called pi_lagged (pi chosen on previous transaction, except the 1st)
resp = unique(pay50$id) # list of respondents names
length(resp)
head(resp)
...
head(pay50[ , c("id", "pi", "pi_lagged", "serial")], 50) # verify
#
...
pay52 = subset(pay51, select = c(id, pi, pi_lagged, amnt, merch, date, time, year, hh_size, age, hh_income, gender, work, marital, education, serial, serial_max))

# changing some chr var to factor
str(pay52)
pay52$id = as.factor(pay52$id)
...
# NNET does not give p-values or significance. Use AER library Wald's z test
coeftest(pay_multi)# all lagged and log(amnt) are ***
