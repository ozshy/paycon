# paycon_3pi_210204.R  revising id
# paycon_3pi_200524.R labeling vertical axis in Figure 2 (Larry's final comments)
...
pay3 = subset(pay2, id %in% resp_adoption_3)
length(unique(pay3$id))
nrow(pay3)
#
nrow(pay3) - nrow(pay2)# loss of trans by restricting to CC and DC adopters
length(unique(pay3$id)) - length(unique(pay2$id))# loss of respondents by restricting to CC and DC adopters
...
trans_by_resp = pay4 %>% group_by(id) %>% summarise("vol" = n())# Num trans made by each resp
head(trans_by_resp)
...
length(unique(trans_by_resp3$id))# num resp who made 3+ payments
...
pay_cash3 = pay53 %>%  group_by(id) %>% summarise(sum(pi == "cash"))
pay_credit3 = pay53 %>%  group_by(id) %>% summarise(sum(pi == "credit"))
pay_debit3 = pay53 %>%  group_by(id) %>% summarise(sum(pi == "debit"))

### Total dollar spending (amnt) by each resp
pay_amnt3 = pay53 %>% group_by(id) %>% summarise(sum(amnt))
head(pay_amnt3)

### Num transactions by each respondent
pay_trans3 = pay53 %>% group_by(id) %>% summarise(n())
head(pay_trans3)
summary(pay_trans3)
table(pay_trans3$`n()`)
sum(pay_trans3$`n()`)
nrow(pay53)
length(unique(pay_trans3$id))# num all respondents with both cards
length(unique(pay53$id))# num all respondents with both cards
...
head(pay_pi33b)
#
names(pay_pi33b) = c("id", "total","cash", "credit", "debit",   "amount")
head(pay_pi33b)
...
length(unique(pay_pi34$id))# num resp in top of Fig 1
sum(pay_pi34$total)# num trans of the above resp
summary(pay_pi34$hhi)
length((pay_pi34[pay_pi34$hhi == 1, ]$id))# num resp with HHI=1
length(unique(pay_pi34[pay_pi34$hhi == 1, ]$id))# num resp with HHI=1 # same as above, showing unique is not needed here
# below percentage of respondents with HHI=1
percent(length((pay_pi34[pay_pi34$hhi == 1, ]$id)))/length(unique(pay_pi34$id))# 
percent(length((pay_pi34[pay_pi34$hhi >= 0.95, ]$id)))/length(unique(pay_pi34$id))# 


# some stats of the data plotted in Figure 1 (resp who made 3+ transactions)
# Info for the top panel of Figure 1 (all HHI)
length(unique(pay_pi34$id))# num resp
...
# Info for HHI = 1 
names(pay_pi34)
pay43_hhi1 = pay43[pay43$id %in% pay_pi34[pay_pi34$hhi == 1,]$id, ]
nrow(pay43_hhi1)# total num trans for HHI=1
length(unique(pay43_hhi1$id))# num resp with HHI=1
...
length(unique(pay_pi34_28$id))# num resp who made less than 29 payments. 
length(unique(pay_pi34$id))-length(unique(pay_pi34_28$id))# num resp who made more than 28 payments. 
#
...
#
par(mar = c(4,4,0.5,1))# plot Figure 2
...
amount_cash3 = pay54_cash %>%  group_by(id) %>% summarise("cash_amount"=sum(amnt))
head(amount_cash3)
pay54_credit = subset(pay53, pi=="credit")
amount_credit3 = pay54_credit %>%  group_by(id) %>% summarise("credit_amount"=sum(amnt))
head(amount_credit3)
pay54_debit = subset(pay53, pi=="debit")
amount_debit3 = pay54_debit %>%  group_by(id) %>% summarise("debit_amount"=sum(amnt))
head(amount_debit3)
#
...
head(pay_pi37)
#
pay_pi38 = merge(x = pay_pi37, y = demog3, by="id",  all.x = T)
head(pay_pi38)
tail(pay_pi38)
...
length(unique(pay53$id)) #verify num resp = nrow pay_pi39
#
(num_payments = nrow(pay53))
(num_resp = length(unique(pay53$id)))
...
(num_resp_trim = length(unique(pay_trim$id)))
