...
setwd("~/Papers/paycon/paycon_coding") # Set working directory!
dir()

### Reading RDS dataset
pay1 = readRDS("paycon_16_17_18.rds")# read diary transations
resp_adoption_3.rds = readRDS("resp_adoption_3.rds")# list of respondents who adopted cash, credit, debit for 3 years
resp_adoption_5.rds = readRDS("resp_adoption_5.rds")# list of respondents who adopted cash, credit, debit, check, prepaid for 3 years
objects()
...
length(resp_adoption_5.rds)
#
pay3 = subset(pay2, uasid %in% resp_adoption_5.rds)
dim(pay3)
...
length(unique(pay3$uasid)) - length(unique(pay2$uasid))# loss of resp by restriction to adoption of all 5 PI
...
#
(fivepi_resp_cash = length(unique(pay4[pay4$pi == 1,]$uasid)))# num resp who pay with cash at least once
(fivepi_resp_check = length(unique(pay4[pay4$pi == 2,]$uasid)))# num resp who pay with check at least once
(fivepi_resp_credit = length(unique(pay4[pay4$pi == 3,]$uasid)))# num resp who pay with credit at least once
(fivepi_resp_debit = length(unique(pay4[pay4$pi == 4,]$uasid)))# num resp who pay with debit at least once
(fivepi_resp_prepaid = length(unique(pay4[pay4$pi == 5,]$uasid)))# num resp who pay with pp at least once
(fivepi_resp_all = length(unique(pay4$uasid)))# num resp who uses PI at least once (incidence)
#
(fivepi_resp_perc_cash = length(unique(pay4[pay4$pi == 1,]$uasid))/fivepi_resp_all)# % resp who pay with cash at least once
(fivepi_resp_perc_check = length(unique(pay4[pay4$pi == 2,]$uasid))/fivepi_resp_all)# % resp who pay with check at least once
(fivepi_resp_perc_credit = length(unique(pay4[pay4$pi == 3,]$uasid))/fivepi_resp_all)# % resp who pay with credit at least once
(fivepi_resp_perc_debit = length(unique(pay4[pay4$pi == 4,]$uasid))/fivepi_resp_all)# % resp who pay with debit at least once
(fivepi_resp_perc_prepaid = length(unique(pay4[pay4$pi == 5,]$uasid))/fivepi_resp_all)# % resp who pay with pp at least once
(fivepi_resp_perc_all = length(unique(pay4$uasid))/fivepi_resp_all)# %S resp who uses PI at least once (incidence)
# Resp incidence for 5+ trans
trans_per_resp = pay4 %>% group_by(uasid) %>% summarise("vol" = n())# Num trans made by each resp
head(trans_per_resp)
...
length(unique(trans_per_resp5$uasid))# num resp who made 5+ payments
sum(trans_per_resp5$vol)# total num trans made by the above resp
pay45 = subset(pay4, uasid %in% trans_per_resp5$uasid)## Restricting pay4 to resp with 5+ trans
...
pay_cash5 = pay55 %>%  group_by(uasid) %>% summarise(sum(pi == "cash"))
pay_check5 = pay55 %>%  group_by(uasid) %>% summarise(sum(pi == "check"))
pay_credit5 = pay55 %>%  group_by(uasid) %>% summarise(sum(pi == "credit"))
pay_debit5 = pay55 %>%  group_by(uasid) %>% summarise(sum(pi == "debit"))
pay_prepaid5 = pay55 %>%  group_by(uasid) %>% summarise(sum(pi == "prepaid"))

### Total dollar spending (amnt) by each resp
pay_amnt5 = pay55 %>% group_by(uasid) %>% summarise(sum(amnt))
head(pay_amnt5)

### Num transactions by each respondent
pay_trans5 = pay55 %>% group_by(uasid) %>% summarise(n())
head(pay_trans5)
summary(pay_trans5)
table(pay_trans5$`n()`)
sum(pay_trans5$`n()`)
nrow(pay55)
length(unique(pay_trans5$uasid))# num all respondents with both cards
length(unique(pay55$uasid))# num all respondents with both cards
...
head(pay_pi53b)
#
names(pay_pi53b) = c("uasid", "total","cash", "check ", "credit", "debit", "prepaid",  "amount")
head(pay_pi53b)
...
length(unique(pay_pi54$uasid))# num resp in top of Fig 1
sum(pay_pi54$total)# num trans of the above resp
summary(pay_pi54$hhi)
length((pay_pi54[pay_pi54$hhi == 1, ]$uasid))# num resp with HHI=1
percent(length((pay_pi54[pay_pi54$hhi == 1, ]$uasid)))/length(unique(pay_pi54$uasid))# 

length(unique(pay_pi54$uasid))# num resp
...
# Info for HHI = 1 
names(pay_pi54)
pay45_hhi1 = pay45[pay45$uasid %in% pay_pi54[pay_pi54$hhi == 1,]$uasid, ]
nrow(pay45_hhi1)# total num trans for HHI=1
length(unique(pay45_hhi1$uasid))# num resp with HHI=1
